...
export function PppDetailPage() {
  ...
  const { data: tarefa, isLoading: tarefaLoading } = useTarefa(id);
  const { data: perguntas, isLoading: perguntasLoading } = usePerguntas(id);
  ...
  if (tarefaLoading || perguntasLoading || respostasLoading || revisoesLoading) {
    return <FullPageLoader message="Carregando PPP..." />;
  }

  return (
    <div className="ppp-detail">
      <PageHeader
        title={tarefa?.nome ?? `PPP #${id}`}
        description="Complete as etapas do PPP e acompanhe a validação."
        actions={(
          <label className="ppp-detail__gt">
            <span>GT</span>
            <select
              value={selectedGtId ?? ''}
              onChange={(event) => {
      ...
      <div className="ppp-detail__lista">
        {blocos.map((bloco) => (
          <Card key={bloco.pergunta.id}>
            <div className="ppp-detail__card">
              ...
              <div
                className="ppp-detail__texto"
                dangerouslySetInnerHTML={{ __html: bloco.pergunta.texto }}
              />

              <div className="ppp-detail__checklist">
                <label>
              ...
              <div className="ppp-detail__actions">
                {bloco.resposta ? (
        ...
        {blocos.length === 0 && (
          <Card>
            <p>Nenhuma missão PPP disponível para este GT.</p>
          </Card>
        )}
      </div>
    </div>
