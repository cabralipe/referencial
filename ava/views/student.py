...
def acessar_aula(request, curso_slug, aula_id):
    ...
    for atividade in atividades:
        atividade.acesso_bloqueado_para_usuario = not user_can_access_activity(request.user, atividade)
        atividade.aviso_bloqueio = (
            activity_manual_block_message(atividade)
            if atividade.acesso_bloqueado and atividade.acesso_bloqueado_para_usuario
            else ""
        )
...
def responder_atividade(request, curso_slug, aula_id, atividade_id):
    ...
    if not user_can_access_activity(request.user, atividade):
        messages.warning(request, activity_block_message(request.user, atividade))
