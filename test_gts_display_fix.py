...
def test_gts_display_fix():
    print("=== Testando correção para exibição de GTs salvos ===\n")
    ...
    # Testar com usuário normal editando a pergunta
    ...
    if normal_user:
        ...
        # Simular request de edição da pergunta
        ...
        print(f"   Queryset disponível (com correção): {queryset.count()} GTs")
        for gt in queryset:
        ...
        if not gts_fora_queryset:
            print(f"\n🎉 CORREÇÃO FUNCIONOU: Todos os GTs salvos estão visíveis!")
        else:
            print(f"\n❌ CORREÇÃO NÃO FUNCIONOU: Ainda há GTs salvos invisíveis.")
    
    # Testar também com super admin
    super_admin = User.objects.filter(email='temp_super@test.com').first()
    if super_admin:
        print(f"\n👑 Testando com super admin: {super_admin.email}")
